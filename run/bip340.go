...
func schnorrVerify(message, pubkey, sig []byte) (bool, error) {
	if len(pubkey) != 32 {
		return false, fmt.Errorf("the public key must be a 32-byte array, got %d.", len(pubkey))
	} else if len(sig) != 64 {
	...
	return R != nil && hasEvenY(R) && x(R).Cmp(r) == 0, nil
}
